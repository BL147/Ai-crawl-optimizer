...
class FixValidationEngine:
    ...
    def _resolve_target_issue(
        ...
        if isinstance(target_issue, str):
            ...
            if s == "AI_ROBOTS_RESTRICTION" or ("ROBOT" in s and ("RESTRICT" in s or "SECURITY" in s)):
                return TargetIssue(category=IssueCategory.AI_ROBOTS_RESTRICTION, description=target_issue, is_security_restriction=True)
            elif s == "AI_RATE_LIMIT" or ("RATE" in s and ("RESTRICT" in s or ("LIMIT" in s and "AI" in s) or "SECURITY" in s)):
                return TargetIssue(category=IssueCategory.AI_RATE_LIMIT, description=target_issue, is_security_restriction=True)
            elif s == "AI_WAF_CHALLENGE" or ("WAF" in s and ("RESTRICT" in s or ("CHALLENGE" in s and "AI" in s) or "SECURITY" in s)):
                return TargetIssue(category=IssueCategory.AI_WAF_CHALLENGE, description=target_issue, is_security_restriction=True)
            elif s == "AI_CAPTCHA" or ("CAPTCHA" in s and ("RESTRICT" in s or "AI" in s or "SECURITY" in s)):
                return TargetIssue(category=IssueCategory.AI_CAPTCHA, description=target_issue, is_security_restriction=True)
            elif s == "AI_AUTHENTICATION" or "AUTH" in s:
                return TargetIssue(category=IssueCategory.AI_AUTHENTICATION, description=target_issue, is_security_restriction=True)
        ...
        if isinstance(target_issue, dict):
            ...
            for c in IssueCategory:
                if c.value == cat_str:
                    cat = c
                    break
            is_sec = target_issue.get("is_security_restriction") or cat in (
                IssueCategory.AI_ROBOTS_RESTRICTION,
                IssueCategory.AI_RATE_LIMIT,
                IssueCategory.AI_WAF_CHALLENGE,
                IssueCategory.AI_CAPTCHA,

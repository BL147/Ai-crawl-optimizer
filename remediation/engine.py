...
class RemediationEngine:
    ...
    def __init__(
        ...
        raw_key = api_key or os.getenv("GEMINI_API_KEY") or os.getenv("LLM_API_KEY")
    ...
    def remediate(self, crawler_result: Any) -> Dict[str, Any]:
        ...
        fallback_res = self._generate_rule_based_remediation(audit).to_dict()
        fallback_res["ai_generated"] = False
        if self.last_error:
            fallback_res["ai_error"] = self.last_error
        return fallback_res

    def generate_remediation(self, crawler_result: Any) -> RemediationResult:
        """
        Generates remediation and returns the rich RemediationResult model object
        (compatible with format_markdown() and to_dict()).
        """
        res_dict = self.remediate(crawler_result)
        return RemediationResult(
            problem_detected=res_dict.get("problem_detected", ""),
            evidence=res_dict.get("evidence", []),
            why_it_affects_ai_crawling=res_dict.get("why_it_affects_ai_crawling", ""),
            recommended_fix=res_dict.get("recommended_fix", ""),
            code_or_configuration_change=res_dict.get("code_or_configuration_change", ""),
            before_after_example=res_dict.get("before_after_example", {}),
            validation_steps=res_dict.get("validation_steps", []),
            uncertainty=res_dict.get("uncertainty", ""),
            observed_facts=res_dict.get("observed_facts", {}),
        )
        ...
        for model_id in candidate_models:
            try:
                    ...
                    if resp.status == 200:
                        ...
                        if candidates:
                            ...
                            if parts:
                                ...
                                if parsed:
                                    res_dict = self._validate_and_finalize_dict(parsed, audit, raw_response=raw_text)
                                    res_dict["model_used"] = model_id
                                    return res_dict
                ...
                try:
                    ...
                    self.last_error = f"Gemini API error ({he.code}): {msg}"
                except Exception:
                    self.last_error = f"Gemini API HTTP {he.code}: {str(he)}"
                # If 403 (invalid key / permission denied) or 401, stop trying models
                if he.code in (401, 403):
                    break
                # If 404 (model not found) or 429 (quota), continue to next fallback model
                continue
            except Exception as e:
                self.last_error = f"API connection error: {str(e)}"

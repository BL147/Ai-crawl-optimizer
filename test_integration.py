...
def test_controlled_crawler_schema_pipeline():
    ...
    assert any("Discrepancy" in p.get("factor", "") or "Selective" in p.get("factor", "") or "Discrimination" in p.get("category", "") for p in base_score["penalties"]), "Baseline discrepancy must trigger AI Discrepancy penalty"
